...
import { cargarTopCity } from "../../utils/ReloadEnv";
...
export default function SearchCity(props) {
  ...
  useEffect(() => {
    (async () => {
      const lst_ciudades = await cargarTopCity();
  ...
  useEffect(() => {
    if (findCity) {
      ...
      const filteredCities = ciudades.filter((ciudades) => {
        let ciudadLowercase = ciudades.ciu_nombre.toLowerCase();
      ...
      {ciudades.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={(ciudad, rowMap) => (
            <Ciudad ciudad={ciudad} navigation={navigation} />
          )}
        />
      )}
      {isLoading ? (
...
function Ciudad(props) {
  ...
  const { ciu_nombre } = ciudad.item;
  return (
    <TouchableOpacity
      style={styles.touchHigh}
      underlayColor="#fff"
      onPress={() =>
        navigation.navigate("TopCity", {
    ...
    >
      <View style={styles.viewBrokerhood}>
        <View>
          <Text style={styles.brokerhoodName}>{ciu_nombre}</Text>
        </View>
        <Icon
